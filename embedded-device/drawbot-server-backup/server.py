...
def start():
    ...
    responseBody = {"isDrawBotOn": draw_bot.is_on(), "isServoUp":draw_bot.isServoUp(), "isShapeDetectorOn": draw_bot.shouldDetectShape()}
...
def get_daraw_bot_status():
    ...
    if commands_thread is not None and commands_thread.is_alive():
        is_draw_bot_on = True
    responseBody = {"isDrawBotOn": is_draw_bot_on, "isServoUp":draw_bot.isServoUp(), "isShapeDetectorOn": draw_bot.shouldDetectShape()}
#     responseBody = {"isDrawBotOn": is_draw_bot_on, "isServoUp": True, "isShapeDetectorOn": False}
...
def socket_thread():
    while True:
        if draw_bot.should_detect_shape():
            ...
            shape_name = labels[np.argmax(probabilities)]
