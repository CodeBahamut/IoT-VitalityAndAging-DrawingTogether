...
def start_shape_detection():
    ...
    if model_is_ready:
        draw_bot.start_moving()
    else:
        image = cv2.resize(camera.value, (224, 224), interpolation=cv2.INTER_AREA)
        image = get_image_ready_for_model(image)    
        probabilities = model.predict(image)
        model_is_ready = True
        start_shape_detection()
...
## testing purposes
@app.route("/draw-bot-state", methods=('POST', 'GET'))
...
def get_drawbot_state():
    return {"isControllerOn": draw_bot.is_controller_on(), "isServoUp":draw_bot.isServoUp(), "isShapeDetectorOn": draw_bot.shouldDetectShape()}

def socket_thread():
    while True:
        if draw_bot.shouldDetectShape(): 
            # Detect color for collision detection
            # Resize the raw image into (224-height,224-width) pixels.
            image_color = cv2.resize(camera.value, (224, 224), interpolation=cv2.INTER_AREA)
            
            global model_is_ready
            if model_is_ready:
                color_name = draw_bot.detect_color(image_color)
                if color_name == "Red":
                    draw_bot.turn_180()
                else:
                    image = get_image_ready_for_model(image_color)
                    probabilities = model.predict(image)
                    index = np.argmax(probabilities)
                    shape_name = labels[index]
                    socketio.emit('onShapeIsDetected', {'shapeName': shape_name})
                    socketio.sleep(0.01)
                    if shape_name.strip() == "Circle":
                        draw_bot.drawSunflower()
                        draw_bot.start_moving()
                    elif shape_name.strip() == "Rectangle":
                        draw_bot.drawTurnLeaf()
                        draw_bot.start_moving()
#                     elif shape_name.strip() == "Triangle":
#                         draw_bot.drawLeaf()
#                         draw_bot.drawLeaf()
#                         draw_bot.start_moving()
        elif draw_bot.is_moving(): ## check whether the Drawbot is moving, we need to stop it 
            draw_bot.stop_moving()
        
        
def get_image_ready_for_model(image): 
